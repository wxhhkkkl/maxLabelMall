...
defineOptions({ name: 'OaContactCategorySelect' })

const props = withDefaults(
  defineProps<{
    modelValue?: number
    categories?: ContactCategoryApi.OaContactCategoryVO[]
    disabled?: boolean
    clearable?: boolean
    filterable?: boolean
    placeholder?: string
...
const categoryList = ref<ContactCategoryApi.OaContactCategoryVO[]>([]) // 分类列表
...
async function getCategoryList() {
  ...
  loading.value = true
  try {
    categoryList.value = await ContactCategoryApi.getSimpleContactCategoryList()
  } finally {
    loading.value = false
  }
}

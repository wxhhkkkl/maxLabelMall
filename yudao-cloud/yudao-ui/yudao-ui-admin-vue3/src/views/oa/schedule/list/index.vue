  ...
  <ContentWrap>
    <el-table v-loading="loading" :data="list">
      ...
      <el-table-column label="参与人" min-width="150" show-overflow-tooltip>
        <template #default="scope">
          {{ scope.row.participantUserNames?.join('、') || '-' }}
        </template>
      </el-table-column>
      <el-table-column label="发布人" prop="creatorName" min-width="100" />
      <el-table-column label="部门" prop="creatorDeptName" min-width="120" show-overflow-tooltip />
      <el-table-column label="发布时间" prop="createTime" :formatter="dateFormatter" width="180" />
      <el-table-column label="提醒" width="70">
        <template #default="scope">
      ...
      <el-table-column label="操作" align="center" width="140" fixed="right">
        <template #default="scope">
          <el-button
            v-if="scope.row.creator === String(userStore.getUser.id)"
            type="primary"
            link
...
const list = ref<ScheduleApi.OaScheduleVO[]>([]) // 日程列表
